...
function initSidebarState() {
  ...
  // Browser erzwingt Neuberechnung von Grid
  layout.offsetHeight;
...
function updateProgressSidebar() {
  // Theorie
  const done_theorie = localStorage.getItem("theorie_done");
  const p_theorie = document.getElementById("progress-theorie");

  if (!p_theorie) return;

  if (done_theorie) {
    p_theorie.textContent = "Theorie ✔";
    p_theorie.style.color = "green";
  }

  // Umsetzung
  const done_umsetzung = localStorage.getItem("umsetzung_done");
  const p_umsetzung = document.getElementById("progress-umsetzung");

  if (!p_umsetzung) return;

  if (done_umsetzung) {
    p_umsetzung.textContent = "Umsetzung ✔";
    p_umsetzung.style.color = "green";
  }

  // Anwendung
  const done_anwendung = localStorage.getItem("anwendung_done");
  const p_anwendung = document.getElementById("progress-anwendung");

  if (!p_anwendung) return;

  if (done_anwendung) {
    p_anwendung.textContent = "Anwendung ✔";
    p_anwendung.style.color = "green";
  }
}
